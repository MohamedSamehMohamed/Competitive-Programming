...
int main(){
  ...
  LCA lca(edges, 2 * n + 4);
  lca.run_dfs();
  lca.get_farthest_two_nodes();
  int diameter = lca.get_diameter();

  int new_node = 4;
  auto two_nodes = lca.get_two_farthest_nodes();

  while (n--){
    int v;
    scanf("%d", &v);
    v--;
    lca.add_to_tree(v, new_node++);
    lca.add_to_tree(v, new_node++);
    for (int i = 0; i < 2; i++){
      int dist = lca.dist(new_node-1, two_nodes[i]);
      if (dist > diameter){
        diameter = dist;
        two_nodes[1-i] = new_node-1;
      }
    }
    lca.update_two_farthest_nodes(two_nodes[0], two_nodes[1]);
    printf("%d\n", diameter);
  }
}